...
// PullModel streams `ollama pull` progress for a model through the Olla gateway's
// Ollama-compatible proxy. Events are written to ch until completion.
func (c *OllaClient) PullModel(model string, ch chan<- PullEvent) {
	defer close(ch)
	raw, _ := json.Marshal(map[string]any{"name": model, "stream": true})
	ctx, cancel := context.WithTimeout(context.Background(), 60*time.Minute)
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodPost,
		c.base+"/olla/ollama/api/pull", bytes.NewReader(raw))
	if err != nil {
		ch <- PullEvent{Err: err, Done: true}
		return
	}
	req.Header.Set("Content-Type", "application/json")
	resp, err := c.http.Do(req)
	...
	if resp.StatusCode >= 400 {
		b, _ := io.ReadAll(io.LimitReader(resp.Body, 512))
		ch <- PullEvent{Err: fmt.Errorf("pull %d: %s", resp.StatusCode, strings.TrimSpace(string(b))), Done: true}
		return
	}
	sc := bufio.NewScanner(resp.Body)
	sc.Buffer(make([]byte, 0, 64*1024), 1024*1024)
	for sc.Scan() {
		...
		if obj.Error != "" {
			ch <- PullEvent{Err: fmt.Errorf("%s", obj.Error), Done: true}
			return
		}
		ch <- PullEvent{Status: obj.Status, Completed: obj.Completed, Total: obj.Total}
	}
	if err := sc.Err(); err != nil {
		ch <- PullEvent{Err: err, Done: true}
		return
	}
	ch <- PullEvent{Status: "success", Done: true}
}

// DeleteModel removes a model from the pool through the gateway proxy.
func (c *OllaClient) DeleteModel(model string) error {
	raw, _ := json.Marshal(map[string]any{"name": model})
	ctx, cancel := c.ctx(60 * time.Second)
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodDelete,
		c.base+"/olla/ollama/api/delete", bytes.NewReader(raw))
	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
